...
pub fn data_graph() {
    ...
    let root_area = BitMapBackend::new("./image.png", (1920, 1080)).into_drawing_area();
    ...
    let mut ctx = ChartBuilder::on(&root_area)
        ...
        .build_cartesian_2d(0.0..duration + 1.0, 0..cpu_count-1)
        .unwrap();

    ctx.configure_mesh().disable_mesh().draw().unwrap();
    ctx.configure_mesh().x_desc("Duration: seconds").y_desc("Cpu core count").draw().unwrap();
    ...
    for (core, switch_events) in data {
        ...
        for item in switch_events.windows(2) {
            ...
            // ranges.push((prev_pos, new_pos, pid_color[&item[1].pid]));
            // prev_pos = new_pos;
        }
        // println!("{}", core);
        // dbg!(&ranges);
        ...
        ctx.draw_series(ranges.iter().map(|y| {
            ...
            println!("{} {} {:?} {}", y.0, y.1, y.2, core);
            bar.set_margin(4, 4, 1, 1);
            bar
        }))
        .unwrap();
    }
